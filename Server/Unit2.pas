...
pathStr:=GetCurrentDir+'\images\';
if not DirectoryExists(pathStr) then
  CreateDir(pathStr);
pathStr:=pathStr+buttonId+'.bmp';
pathImg:=OpenPictureDialog1.FileName;
CopyFile(PWideChar(pathImg),PWideChar(pathStr),True);
editurl.Text:=pathStr;
end;
...
case id[1] of
  ...
  begin
   ...
   if sizeY[tempInt]<>1 then
    for j := 1 to sizeY[tempInt]-1 do  //DESNECESSARIO
     begin
